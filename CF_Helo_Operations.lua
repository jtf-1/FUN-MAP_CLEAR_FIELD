...
function SEF_INFANTRY_SmokeLoadingZones()
	...
	if (InfantrySmokeLockout == 0) then
		...
		timer.scheduleFunction(SEF_INFANTRY_LoadingZoneSmokeUnlock, nil, timer.getTime() + 300)
...
function SEF_INFANTRY_SmokeExtractionZones()
	...
	if ( InfantryExtractionSmokeLockout == 0 and #InfantryTable > 0 ) then		
		...
		timer.scheduleFunction(SEF_INFANTRY_ExtractionZoneSmokeUnlock, nil, timer.getTime() + 300)	
...
function SEF_STINGER_SmokeExtractionZones()
	...
	if ( StingerExtractionSmokeLockout == 0 and #StingerTable > 0 ) then		
		...
		timer.scheduleFunction(SEF_STINGER_ExtractionZoneSmokeUnlock, nil, timer.getTime() + 300)	
...
function SEF_USACE_SmokeExtractionZones()
	...
	if ( USACEExtractionSmokeLockout == 0 and #USACETable > 0 ) then		
		...
		timer.scheduleFunction(SEF_USACE_ExtractionZoneSmokeUnlock, nil, timer.getTime() + 300)	
...
function SEF_CARGO_SmokeLoadingZones()
	...
	if ( CargoContainerSmokeLockout == 0 ) then		
		...
		timer.scheduleFunction(SEF_CARGO_LoadingZoneSmokeUnlock, nil, timer.getTime() + 300)
...
function SEF_UPDATECONSTRUCTIONTABLE()
	...
	if #USACEPALADINTable > 0 then
		for i = 1, #USACEPALADINTable do
			...
			ClearFieldMkIIConstructions[TableConstructor] = {}
			ClearFieldMkIIConstructions[TableConstructor].Type = "Paladin"
			ClearFieldMkIIConstructions[TableConstructor].Vec2X = ConstructedGroupVec2.x
			ClearFieldMkIIConstructions[TableConstructor].Vec2Y = ConstructedGroupVec2.y
